...
module.exports = {
    ...
    sendRequest(host,data,callback1){
        // var result = qs.parse(data,'\n',':');
        var method = data.split('\n')[0].split(' ')[0];
        var hostname = host.split(':')[0];
        var port = host.split(':')[1];
        var path = data.split('\n')[0].split(' ')[1];
        var contents = '';

        var header_arr = data.split('\n\n')[0];
        var header_arr2 = header_arr.split('\n');
        var header_arr3 = header_arr2.slice(0);
        header_arr3.shift(header_arr2[0]);
        var header_arr4 = header_arr3.join('\n');
        var headers = qs.parse(header_arr4,'\n',':');
        // if (method==='POST'){
            var arr = data.split('\n\n');
            var content = arr.slice(0);
            content.shift();
        ...
        var req_func = function (options, contents ,callback) {
        var req = http.request(options, function (res) {
            var response = 'HTTP/' + res.httpVersion + ' ' + res.statusCode + ' ' + res.statusMessage + '\n';
        ...
        req_func(options, contents, function (err, response, data) {
            callback1(response + '\n\n' + data);
        })
    },
    /**
