<template>
    <div id="network">
        <el-table
            ...
            <el-table-column label="重放" width="">
                <template slot-scope="scope">
                   <el-button size="mini" type="primary" icon="d-arrow-right" @click="handleRequest(scope.$index, scope.row)"></el-button>
                </template>
            </el-table-column>
            <el-table-column
            type="index"
...
export default {
    ...
    methods: {
        ...
        handleRequest(index, row) {
            this.$remoteApi.getSingleLog(row.id).then((rec) => {
                this.transContent = rec;
                this.$router.push({
                    path: '/request',
                    name:"request",
                    params: {
                        data: JSON.stringify(this.transContent),
                    }
                });
            }, () => {});
        // console.log(this.transContent.id);
